...
Agent-CodeRAG achieves comparable precision to Full Context while using 5\% of tokens. Notably, it reduces token usage by 95\% compared to full file inclusion, and the delta-sync mechanism reduces distillation API costs by 92\% on average across incremental updates.
...
This work was supported by the OpenClaw research initiative. The code is open-source and available at \url{https://github.com/naranor/agent-coderag} under the MIT license. Experiments were conducted on the CodeSearchNet benchmark; data and scripts are included in the supplementary material.
