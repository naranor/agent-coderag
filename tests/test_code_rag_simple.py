...
async def test_coderag_sync_and_search(temp_db):
    ...
    mock_embedder.embed.side_effect = lambda texts: np.random.rand(len(texts), 384).astype(np.float32)
    ...
    mock_distiller.summarize = AsyncMock(return_value="This is a test function for RAG.")
