...
class JavaParser(IParser):
        ...
        try:
            ...
            if hasattr(tree, "imports"):
                for imp in tree.imports:
                    relations.append(
                        Relation(
                            from_id=module_id, to_id=imp.path, type=RelationType.IMPORTS
                        )
                    )
            ...
            for _, node in tree.filter(javalang.tree.ClassDeclaration):
                ...
                for method in node.methods:
                    units.append(
                        self._parse_method(method, node.name, file_path, lines)
                    )

            # Extract interfaces
            for _, node in tree.filter(javalang.tree.InterfaceDeclaration):
                units.append(
                    self._parse_class(node, file_path, lines, is_interface=True)
                )
    ...
    def _parse_class(
        self, node: Any, file_path: str, lines: List[str], is_interface: bool = False
    ) -> KnowledgeUnit:
        ...
        return KnowledgeUnit(
            ...
            path=file_path,
            code_hash=node_hash,
            metadata={"is_interface": is_interface, "raw_code": node_source},
        )

    def _parse_method(
        self, node: Any, class_name: str, file_path: str, lines: List[str]
    ) -> KnowledgeUnit:
